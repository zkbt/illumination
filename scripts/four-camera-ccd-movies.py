...
i = FourCameraOfCCDsIllustration(**cameras)
i.plot()
filename = 'orbit{}-four-camera.pdf'.format(orbit)
i.savefig(filename, dpi=300)
i.animate(filename.replace('pdf', 'mp4'), cadence=1.0*u.s)

# make both a raw, and a differenced movie
for cam in cameras.keys():
    i = CameraOfCCDsIllustration(**cameras[cam])
    i.plot()
    filename = 'orbit{}-{}.pdf'.format(orbit, cam)
    i.savefig(filename, dpi=300)
    i.animate(filename.replace('pdf', 'mp4'), cadence=1.0*u.s)
