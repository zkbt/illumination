...
class Cube(Talker):
    ...
    def __init__(
        ...
        try:
            ...
            self.time = (
                Time("2018-01-01 00:00:00.000").gps + np.arange(self.n) * self.cadence
            )
    ...
    def stack(self, cadence, strategy=Central(n=10), plot=False):
        ...
        nsubexposures = int(cadence / self.cadence)

        # loop over the x and y pixels
        self.speak(
            "binning {0} cube by {1} subexposures".format(self.shape, nsubexposures)
        )
        ...
        binned = Cube(
            array, cadence, temporal=temporal, spatial=self.spatial, static=self.static
        )
    ...
    def imshow(self, timestep=0, nsigma=0.5):
        ...
        plotted = plt.imshow(
            image, interpolation="nearest", origin="lower", norm=norm, cmap=cmap
        )
        ...
        colorbar = plt.colorbar(
            ...
            label=self.colorbarlabelfordisplay(0),
            fraction=0.04,
            pad=0.02,
            ticks=ticks,
        )
    ...
    def median(self, which="photons"):
        ...
        The median image.
        """
        array = self.__dict__[which].astype(float)
        key = "median"
        try:
    ...
    def mad(self, which="photons"):
        ...
        try:
            ...
            self.summaries[key + which] = np.median(
                np.abs(array - self.cubify(self.median(which))), timeaxis
            )
        return self.summaries[key + which]
    ...
    def oneWeirdTrickToTrimCosmics(self, threshold=4):
        ...
        shape[timeaxis] = 1
        bad = self.photons > (
            self.median() + threshold * self.sigma(robust=True)
        ).reshape(shape)
        x, y, z = bad.nonzero()
    ...
    def nsigma(self, which="photons", robust=True):
        ...
        """
        array = self.__dict__[which].astype(float)
        return (array - self.cubify(self.median(which))) / self.cubify(
            self.sigma(which, robust=robust)
        )
    ...
    def write(self, normalization="none", directory="cube"):
        ...
        for i in range(self.n):
            ...
            filename = os.path.join(
                directory, normalization + "_{0:05.0f}.fits".format(i)
            )
    ...
    def plot(
        self, ax=None, normalization="none", ylim=[None, None], color="gray", **kw
    ):
        ...
        if ax is None:
            plt.figure(
                ...
                figsize=(self.xpixels * scale, self.ypixels * scale),
                dpi=72,
            )
            gs = plt.matplotlib.gridspec.GridSpec(
                self.ypixels, self.xpixels, wspace=0, hspace=0
            )
            ax = {}
            a = None
        ...
        for i in range(self.ypixels):
            for j in range(self.xpixels):
                ax[(i, j)].plot(
                    self.time, photonsnormalized[:, i, j], color=color, **kw
                )
