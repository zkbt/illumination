...
def create_test_stamp(
    col_cent=3900, row_cent=913, cadence=2, cam=1, spm=1, tic_id=1234567890, **kw
):
    static = {
        "CAM": cam,
        "COL_CENT": col_cent,
    ...
    temporal["QUAL_BIT"] = np.zeros(N).astype(int)
...
def create_test_fits(
    rows=400, cols=600, circlescale=100, visualize=False, noise=0.2, seed=None
):
    """
    ...
    x, y = np.meshgrid(np.arange(cols).astype(float), np.arange(rows).astype(float))
