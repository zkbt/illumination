...
def add_zoom(illustration, position, size=(25, 25), zoom=5, frame=None):
    """
    ...
    if frame is None:
        if "camera" in illustration.frames:
            frame = "camera"
        elif "image" in illustration.frames:
            frame = "image"
        else:
            frame = list(illustration.frames.keys())[0]
    reference_frame = illustration.frames[frame]

    # define a key for this frame
    key = "zoom-{}-({},{})".format(frame, position[0], position[1])
    illustration.frames[key] = LocalZoomFrame(
...
def add_grid_of_zooms(
    illustration, N=4, buffer=100, cameras=[1, 2, 3, 4], zoom=10, size=10, **kw
):
    """
...
def add_stamp(illustration, stamp, zoom=5, camera="camera"):
    ...
    illustration.frames[key] = LocalStampFrame(
        illustration=illustration,
        ax=None,
        data=stamp,
        source=reference_frame,
