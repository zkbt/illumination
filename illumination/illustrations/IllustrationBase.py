...
class IllustrationBase(Talker):
    ...
    def __init__(
        ...
        **gridspeckw : dict
        ...
        if subplot_spec is not None:
            ...
            subset = {
                k: gridspeckw.get(k)
                for k in ["wspace", "hspace", "height_ratios", "width_ratios"]
            }
            self.grid = gs.GridSpecFromSubplotSpec(
                nrows, ncols, subplot_spec=subplot_spec, **subset
            )
    ...
    def __repr__(self):
        ...
        """
        return "<{} Illustration | ({} Frames)>".format(
            self.illustrationtype, len(self.frames)
        )
    ...
    def _cmap_norm_ticks(self, remake=False, **cmapkw):
        ...
        **kwargs are passed to colors.cmap_norm_ticks
        ...
        try:
            ...
            (self.plotted["cmap"], self.plotted["norm"], self.plotted["ticks"])
            ...
            (self.plotted["cmap"], self.plotted["norm"], self.plotted["ticks"]) = (
                cmap_norm_ticks(np.asarray(firstimages), **cmapkw)
            )
            self.speak(
                "defined color scheme with \n cmap={}\n norm={}\n ticks={}".format(
                    self.plotted["cmap"], self.plotted["norm"], self.plotted["ticks"]
                )
            )
        return (self.plotted["cmap"], self.plotted["norm"], self.plotted["ticks"])
    ...
    def _add_colorbar(self, imshowed, ax=None, ticks=None):
        ...
        self.speak(
            "adding a new colorbar for {} frame(s)".format(len(np.atleast_1d(ax)))
        )
    ...
    def animate(
        ...
        **kw,
        ...
        if mintime is None:
            ...
            lower, upper = (
                min(actualtimes.gps),
                max(actualtimes.gps) + actualcadence.to("s").value,
            )
        else:
            lower = mintime.gps
        ...
        self.speak(
            "about to animate {} times at {}s cadence for {}".format(
        ...
        with writer.saving(self.figure, filename, dpi or self.figure.get_dpi()):
            for i, t in enumerate(times):
                self.speak(
                    "  {}/{} at {}".format(i + 1, len(times), Time.now().iso),
                    progress=True,
                )
